...
for i = 1:length(SNR_vec)
    ...
    for j = 1:length(m)
        ...
        for l = 1:montecarlo_N
            ...
            total_BER = total_BER + markov_simulate_FSMC(previous_g, stay_g, next_g, error_probs_g, previous_b, stay_b, next_b, error_probs_b, N_bits, good_bits_start, good_bits_end);
        end
        BER_vec(i, j) = total_BER/montecarlo_N;
        if m(j) == 1
            bad_BER = 0;
            for l = 1:montecarlo_N
                bad_BER = bad_BER + markov_simulate(previous_b, stay_b, next_b, error_probs_b, N_bits);
            end
            just_b(i) = bad_BER/montecarlo_N;
        elseif m(j) == 15
            good_BER = 0;
            for l = 1:montecarlo_N
                good_BER = good_BER + markov_simulate(previous_g, stay_g, next_g, error_probs_g, N_bits);
            end
...
hold on
for i = 2:length(m)
    throughput_norm = throughput(64, 64, BER_vec(:,i));
    plot(SNR_vec, throughput_norm, '*-', 'DisplayName', sprintf('Simulated Nakagami FSMC m = %d', m(i)))
end
bad_throughput = throughput(64, 64, just_b);
good_throughput = throughput(64, 64, just_g);
awgn_throughput = throughput(64, 64, awgn_channel);
