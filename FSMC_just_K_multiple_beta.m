...
for m = 1:length(beta_vals)
    ...
    for i = 1:length(SNR_vec)
        ...
        thresholds = threshold_calc_gen(n_states, 1e-6, @(z) k_SNR_cdf_an(z, EsNo, beta_val), max(3*EsNo*abs(beta_val),1));
    ...
    semilogy(SNR_vec, analytical', style_an(m), 'DisplayName', 'Analytical K-fading')
    semilogy(SNR_vec, analytical_gen', style_gen(m), 'DisplayName', ['Analytical Gen. K-fading (beta=', num2str(beta_val), ')'])
...
save_name = 'ber_FSMC_k_fading_many_betas_gen';
